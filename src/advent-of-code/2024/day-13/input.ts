...
import { Button, ClawMachine, Prize } from "./Solution";

const SMALL = `Button A: X+94, Y+34
Button B: X+22, Y+67
Prize: X=8400, Y=5400
...
const BUTTON_PATTERN = /X\+(\d+), Y\+(\d+)/;
const PRIZE_PATTERN = /X=(\d+), Y=(\d+)/;

const parseButton = (buttonLine: string): Button => {
  const match = buttonLine.match(BUTTON_PATTERN);
  if (!match) throw new Error("Button can't be parsed");
  return { dx: Number.parseInt(match[1]), dy: Number.parseInt(match[2]) };
};

const parsePrize = (prizeLine: string): Prize => {
  const match = prizeLine.match(PRIZE_PATTERN);
  if (!match) throw new Error("Prize can't be parsed");
  return { x: Number.parseInt(match[1]), y: Number.parseInt(match[2]) };
};

const parseInput = (input: string): ClawMachine[] => {
  let l = lines(input);
  let result: ClawMachine[] = [];

  while (l.length > 0) {
    const [buttonALine, buttonBLine, prizeLine, , ...rest] = l;
    l = rest;
